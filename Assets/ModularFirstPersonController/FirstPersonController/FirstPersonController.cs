...
// Original version by Jess Case https://assetstore.unity.com/packages/3d/characters/modular-first-person-controller-189884#description
// Modified for this project by Sloan Gwaltney
// Modifications include:
// Replaceing all inputs with overrides for abstraction of inputs. This lets the user use their choice of the old or new input system. Before was incompatible with new input system.
// Made sprint bar, crosshair and other realted game objects not required. Certain branches of code would require crosshair game objects even if player is not using a crosshair and other artifacts
...
public class FirstPersonController : MonoBehaviour
{
    ...
    protected virtual float GetHorizontalLook()
    {
        Debug.LogError("GetHorizontalLook: This method should be overridden is child class");
        return 0;
    }

    protected virtual float GetVerticalLook()
    {
        Debug.LogError("GetVerticalLook: This method should be overridden is child class");
        return 0;
    }

    protected virtual bool IsZoomHeldDown()
    {
        Debug.LogError("IsZoomDown: This method should be overridden is child class");
        return false;
    }

    protected virtual bool IsJumpKeyDown()
    {
        Debug.LogError("IsJumpKeyDown: This method should be overridden is child class");
        return false;
    }

    protected virtual bool IsCrouchKeyDown()
    {
        Debug.LogError("IsCrouchKeyDown: This method should be overridden is child class");
        return false;
    }

    protected virtual float GetHorizontalMovement()
    {
        Debug.LogError("GetHorizontalMovement: This method should be overridden is child class");
        return 0;
    }

    protected virtual float GetVerticalMovement()
    {
        Debug.LogError("GetVerticalMovement: This method should be overridden is child class");
        return 0;
    }

    protected virtual bool IsSprintKeyDown()
    {
        Debug.LogError("IsSprintKeyDown: This method should be overridden is child class");
        return false;
    }
    ...
    private void Update()
    {
        ...
        if (cameraCanMove)
        {
            yaw = transform.localEulerAngles.y + GetHorizontalLook() * mouseSensitivity;

            if (!invertCamera)
            {
                pitch -= mouseSensitivity * GetVerticalLook();
            }
            else
            {
                // Inverted Y
        ...
        if (enableZoom)
        {
            ...
            if (IsZoomHeldDown() && !holdToZoom && !isSprinting)
            {
                if (!isZoomed)
                {
                    isZoomed = true;
        ...
        if (enableJump && IsJumpKeyDown() && isGrounded)
        {
            Jump();
        }
        ...
        if (enableCrouch)
        {
            ...
            else if (!IsCrouchKeyDown() && holdToCrouch)
            {
                isCrouched = true;
                Crouch();
            }
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            Vector3 targetVelocity = new Vector3(GetHorizontalMovement(), 0, GetVerticalMovement());
            ...
            if (enableSprint && IsSprintKeyDown() && sprintRemaining > 0f && !isSprintCooldown)
            {
